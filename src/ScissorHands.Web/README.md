...
For preview, set `Site:BaseUrl` to `/` or a path prefix with leading and trailing slashes, such as `/docs/` or `/manual/docs/`. Open the logged preview URL: pages, assets, and directory redirects resolve beneath that prefix with or without locale routing. Output remains directly under `preview/`, without an extra prefix directory. With a non-root prefix, requests outside that prefix return 404, including `/` and unprefixed asset URLs. A request to `/docs` redirects to `/docs/`; configuring `/` retains normal root hosting. This is a routing boundary, not authentication. Other base-URL forms are not covered by this preview contract. Production hosts must mount `dist/` at the configured prefix themselves.
