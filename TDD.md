...
| Implementation baseline | `4b32a4c` plus the tested #89 prefix-only follow-up in this revision |
...
**Implemented in #89:** per the user's prefix-only clarification, `Map` matches the complete configured path prefix, moves it into `Request.PathBase`, and supplies the artifact-relative `Request.Path` to default/static-file middleware inside the branch. The registration key omits the final slash without changing `Site.BaseUrl` or rendered HTML. Outside-prefix requests do not reach static-file middleware and fall through to 404. The existing default-file middleware preserves the path base and query in directory/mount redirects. `/` registers the same file middleware on the main pipeline and retains root behavior. Documented usage is root or leading/trailing-slash path prefixes such as `/docs/` and `/manual/docs/`; other forms remain outside the guaranteed boundary under TG-001. No custom request-path stripping, general URL normalization, generated-directory nesting or locale change is introduced. This is routing isolation, not authentication.

[Application HTTP tests](test/ScissorHands.Web.Tests/ScissorHandsApplicationTests.cs) use the internal application constructor, an ephemeral Kestrel port, a temporary output root and a fixture generator. Readiness is signalled at watcher creation after the initial build; bounded waits, host shutdown and restoration of the process working directory isolate each `NonParallel` case. Six cases cover three mounts with locale on/off, assets, HEAD, redirects/queries, missing and nonmatching paths, outside-prefix GET/HEAD rejection and artifact inventory. The separate real sample run confirms generation, resolved neighbor requests and prefix-only serving; see [PRD V-003](PRD.md#v-003-89-preview-mount-execution-2026-09-16). #89 remains independent of #81; these checks do not complete the broader V-003 or scheme-policy work.
