...
Preview serves generated files only at the configured path prefix. Use `/` for root hosting, or a prefix with leading and trailing slashes such as `/docs/` or `/manual/docs/`. With `/docs/`, a page route `parent/child` is served at `/docs/parent/child/`, and `ko-kr/parent/child` at `/docs/ko-kr/parent/child/`; theme assets and images use the same mount. Requests outside the prefix, including `/`, `/parent/child/`, and `/docs-other/`, return 404. `/docs` redirects to `/docs/`, and directory redirects retain the prefix and query string. Files remain directly under `preview/`. This removes the former unprefixed aliases for non-root configurations; update preview bookmarks and links to use the configured prefix. It is a routing boundary, not authentication. Other base-URL forms, including absolute URLs, relative paths, or query/fragment-bearing prefixes, are outside this preview contract. No new general URL-validation policy is implied.
